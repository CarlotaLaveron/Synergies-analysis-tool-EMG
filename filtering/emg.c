...
double filter_EMG(EMG *self, double data)
{
    // 1) Escalamos el dato pequeño a entero
    int scaled = (int)lround(data * EMG_SCALE);

    // 2) Filtro LPF si no hay referencia
    if (self->reference_available != REFERENCE_AVAILABLE) {
        scaled = get_moving_average(self->LPF_data, scaled);
    }
    ...
    if (self->remove_low_frequency == HIGH_PASS_FILTER_ON) {
        int neutral_value = get_moving_average(self->For_HPF, scaled);
        rectified = abs(scaled - neutral_value);
    } else {
        rectified = abs(scaled);
    }

    filtered = get_moving_average(self->MA, rectified);

    // 3) Desescalamos antes de devolver (sigue siendo int)
    return (double)filtered / EMG_SCALE;
}

double filter_EMG_r(EMG *self, double data, double reference_data)
{
    if (self->reference_available != REFERENCE_AVAILABLE) {
        return -1;
    }

    // Escalamos ambos canales
    int scaled_data = (int)lround(data * EMG_SCALE);
    int scaled_ref  = (int)lround(reference_data * EMG_SCALE);

    int clean_data      = get_moving_average(self->LPF_data,      scaled_data);
    int clean_reference = get_moving_average(self->LPF_reference, scaled_ref);

    int diff_scaled = clean_data - clean_reference;

    // diff_scaled está ya en el espacio escalado de enteros.
    // Lo pasamos a double (desescalado) para reutilizar filter_EMG
    double diff = (double)diff_scaled / EMG_SCALE;

    return (double)filter_EMG(self, diff);
}
